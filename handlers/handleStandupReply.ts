...
export const getBotFirstMessageByMessageInChannel = async (message: Message): Promise<Message | undefined> => {
  const channel = await client.channels.cache.get(message.channelId);
  if (!channel?.isTextBased()) return;

  const channelMessages: Map<any, Message> = await channel.messages.fetch({ limit: 100 })

  const botMessages = Array.from(channelMessages.values())
    .filter(msg => {
